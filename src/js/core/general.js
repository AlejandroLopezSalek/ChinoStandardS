...
globalThis.AppUtils.Notifications = {
    ...
    async subscribe() {
        try {
            ...
            if (globalThis.ToastSystem) {
                const msg = window.I18N_COMMON?.notifications?.success || '¡Notificaciones activadas! Te avisaremos para que vuelvas a estudiar.';
                const title = window.I18N_COMMON?.notifications?.success_title || 'Éxito';
                globalThis.ToastSystem.success(msg, title, 5000);
            } else {
                alert(window.I18N_COMMON?.notifications?.success || '¡Notificaciones activadas! Te avisaremos para que vuelvas a estudiar.');
            }

        } catch (error) {
            console.error('Subscription failed:', error);
            if (globalThis.ToastSystem) {
                const msg = window.I18N_COMMON?.notifications?.error_subscribe || 'No se pudo activar las notificaciones. Intenta más tarde.';
                const title = window.I18N_COMMON?.notifications?.error_title || 'Error';
                globalThis.ToastSystem.error(msg, title, 5000);
            } else {
                alert(window.I18N_COMMON?.notifications?.error_subscribe || 'No se pudo activar las notificaciones. Intenta más tarde.');
            }
        }
    },
    ...
    async unsubscribe() {
        try {
            ...
            this.updateUI(false);
            if (globalThis.ToastSystem) {
                const msg = window.I18N_COMMON?.notifications?.unsubscribed || 'Notificaciones desactivadas. Ya no recibirás alertas.';
                const title = window.I18N_COMMON?.notifications?.info_title || 'Desactivado';
                globalThis.ToastSystem.info(msg, title, 4000);
            }
        } catch (error) {
            console.error('Error unsubscribing', error);
            if (globalThis.ToastSystem) {
                const msg = window.I18N_COMMON?.notifications?.error_unsubscribe || 'No se pudo desactivar. Intenta de nuevo.';
                const title = window.I18N_COMMON?.notifications?.error_title || 'Error';
                globalThis.ToastSystem.error(msg, title, 4000);
            }
        }
    },
            ...
            newToggle.addEventListener('change', (e) => {
                if (e.target.checked) {
                    ...
                    if (globalThis.AuthService && !globalThis.AuthService.isLoggedIn()) {
                        ...
                        if (globalThis.ToastSystem) {
                            const msg = window.I18N_COMMON?.notifications?.login_required || 'Debes iniciar sesión para activar las notificaciones.';
                            const title = window.I18N_COMMON?.notifications?.warning_title || 'Solo Usuarios Registrados';
                            globalThis.ToastSystem.warning(msg, title, 4000);
                        } else {
                            alert(window.I18N_COMMON?.notifications?.login_required || 'Debes iniciar sesión para activar las notificaciones.');
                        }
                        return;
                    }
                    this.subscribe();
