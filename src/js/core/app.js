    ...
    class PandaLatamApp {
        ...
        handleError(error) {
            ...
            if (!isMinor && globalThis.ToastSystem) {
                const msg = window.I18N_COMMON?.app_messages?.generic_error || 'Ha ocurrido un error. Por favor, intenta de nuevo.';
                const title = window.I18N_COMMON?.app_messages?.error_title || 'Error';
                globalThis.ToastSystem.error(msg, title);
            }
        }
                    ...
                    newWorker.onstatechange = () => {
                        if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
                            ...
                            if (globalThis.ToastSystem) {
                                const msg = window.I18N_COMMON?.app_messages?.sw_update_available || 'Una nueva versión está disponible. Haz clic aquí para actualizar.';
                                const title = window.I18N_COMMON?.app_messages?.sw_update_title || 'Actualización PandaLatam';
                                globalThis.ToastSystem.info(
                                    msg,
                                    title,
                                    () => globalThis.location.reload()
                                );
                            } else if (confirm(window.I18N_COMMON?.app_messages?.sw_update_available || 'Nueva versión disponible. ¿Recargar ahora?')) {
                                globalThis.location.reload();
                            }
                        }
                    };
        ...
        setupPWAInstall() {
            ...
            globalThis.addEventListener('appinstalled', () => {
                ...
                if (globalThis.ToastSystem) {
                    const msg = window.I18N_COMMON?.app_messages?.pwa_installed || '¡App instalada correctamente!';
                    globalThis.ToastSystem.success(msg);
                }
            });
        }
        ...
        initializeGrammarSearch() {
            ...
            globalThis.SearchSystem.createSearchBar(searchContainer, {
                placeholder: window.I18N_COMMON?.app_messages?.search_placeholder || 'Buscar temas de gramática...',
                onSearch: (query) => {
        ...
        showInitError(error) {
            ...
            errorDiv.innerHTML = `
                ...
                <h2 style="margin-bottom: 10px; color: #1e293b;">${window.I18N_COMMON?.app_messages?.init_error_title || 'Error de inicialización'}</h2>
                <p style="color: #64748b; margin-bottom: 20px;">
                    ${window.I18N_COMMON?.app_messages?.init_error_desc || 'No se pudo inicializar la aplicación. Por favor, recarga la página.'}
                </p>
                <button onclick="globalThis.location.reload()" style="
                    ...
                    font-weight: 600;
                ">
                    ${window.I18N_COMMON?.app_messages?.reload_btn || 'Recargar'}
                </button>
            `;
