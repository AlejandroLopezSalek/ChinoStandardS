    ...
    if (isLoggedIn) {
        try {
            ...
            if (authButtonContainer) {
                const welcomeMsg = window.I18N_COMMON?.footer?.welcome || 'Bienvenido/a, ';
                authButtonContainer.innerHTML = `
                    <span style="color: #E0E0E0; font-weight: 500;">
                        ${welcomeMsg}${currentUser.username || currentUser.email}
                    </span>
                `;
            }
...
function showLoginState() {
    ...
    // Show login link
    if (authButtonContainer) {
        const loginLabel = window.I18N_COMMON?.nav?.login || 'Inicia Sesión';
        authButtonContainer.innerHTML = `
            <a href="/login/" 
...
function handleLogout() {
    ...
    if (globalThis.ToastSystem) {
        const msg = window.I18N_COMMON?.footer?.logout_success || 'Sesión cerrada correctamente';
