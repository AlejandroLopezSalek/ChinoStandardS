    ...
    if (globalThis.ContributionService) {
        ...
        setTimeout(() => {
            if (globalThis.ContributionService) {
                ...
                console.error(window.I18N_COMMUNITY?.messages?.service_unavailable || 'ContributionService not available');
            }
        }, 500);
    }
...
function buildBookCard(book) {
    ...
    if (book.isCommunity) {
        ...
        badge.textContent = window.I18N_COMMUNITY?.books?.community_badge || 'Comunidad';
...
function buildBookMetaHtml(book) {
    ...
    if (book.pages) {
        html = `<span class="flex items-center gap-1"><i class="fas fa-file-alt"></i> ${book.pages} ${window.I18N_COMMUNITY?.books?.pages_suffix || 'pág.'}</span>` + html;
    }
...
function buildBookViewBtn(url) {
    ...
    btn.innerHTML = `<i class="fas fa-eye"></i> ${window.I18N_COMMUNITY?.books?.view_btn || 'Ver'}`;
    return btn;
}
...
function buildBookDownloadBtn(url) {
    ...
    btn.innerHTML = `<i class="fas fa-download"></i> ${window.I18N_COMMUNITY?.books?.download_btn || 'Descargar'}`;
    return btn;
}
...
function buildEmptyState() {
    ...
    el.innerHTML = `
        ...
        <p class="text-xl">${window.I18N_COMMUNITY?.lessons?.empty || 'No hay lecciones encontradas'}</p>
    `;
    return el;
}
...
function buildLessonCard(lesson) {
    ...
    header.innerHTML = `
        <div class="flex items-center gap-2 px-3 py-1 rounded-md bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-sm font-bold">
            ...
            <span>${window.I18N_COMMUNITY?.lessons?.lesson_prefix || 'Lección'} ${lesson.level}</span>
        </div>
        <span class="text-xs font-bold uppercase tracking-wider px-2 py-1 rounded bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">
            ${window.I18N_COMMUNITY?.lessons?.status_published || 'Publicada'}
        </span>
    `;
...
function buildLessonActions(lesson) {
    ...
    viewBtn.innerHTML = `<i class="fas fa-eye"></i> ${window.I18N_COMMUNITY?.lessons?.view_btn || 'Ver Lección'}`;
    ...
    moreBtn.setAttribute('aria-label', window.I18N_COMMUNITY?.lessons?.more_options || 'Más opciones');
    ...
    const editBtn = buildDropdownItem(`<i class="fas fa-edit text-red-500"></i> ${window.I18N_COMMUNITY?.lessons?.suggest_edit || 'Sugerir Edición'}`, false, () => {
        editPublishedLesson(lesson.id);
    });
    dropdownMenu.appendChild(editBtn);

    if (globalThis.ContributionService?.isAdmin()) {
        const deleteBtn = buildDropdownItem(`<i class="fas fa-trash"></i> ${window.I18N_COMMUNITY?.lessons?.delete_btn || 'Eliminar'}`, true, () => {
...
function renderPagination(containerId, totalItems, currentPage, onPageChange) {
    ...
    container.appendChild(buildPaginationBtn(
        `<i class="fas fa-chevron-left"></i> ${window.I18N_COMMUNITY?.pagination?.prev || 'Prev'}`,
        currentPage <= 1,
    ...
    // Next button
    container.appendChild(buildPaginationBtn(
        `${window.I18N_COMMUNITY?.pagination?.next || 'Next'} <i class="fas fa-chevron-right"></i>`,
    ...
    return `
        <div class="mb-8 p-4 bg-stone-50 dark:bg-stone-700/50 rounded-xl border border-stone-200 dark:border-stone-700">
            <h3 class="text-sm font-bold uppercase tracking-wider text-stone-500 dark:text-stone-400 mb-3 flex items-center gap-2">
                <i class="fas fa-info-circle"></i> ${window.I18N_COMMUNITY?.modals?.detail?.info_title || 'Información'}
            </h3>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                <div>
                    <strong class="text-stone-700 dark:text-stone-300 block mb-1">${window.I18N_COMMUNITY?.modals?.detail?.level || 'Nivel'}:</strong>
                ...
                <div>
                    <strong class="text-stone-700 dark:text-stone-300 block mb-1">${window.I18N_COMMUNITY?.modals?.detail?.author || 'Autor'}:</strong>
                    <span class="text-stone-600 dark:text-stone-400">${lesson.author || (window.I18N_COMMUNITY?.modals?.detail?.anonymous || 'Anónimo')}</span>
                </div>
                <div>
                    <strong class="text-stone-700 dark:text-stone-300 block mb-1">${window.I18N_COMMUNITY?.modals?.detail?.date || 'Fecha'}:</strong>
        ...
        <div>
            <h3 class="text-lg font-bold text-stone-800 dark:text-stone-100 mb-4 flex items-center gap-2 border-b border-stone-200 dark:border-stone-700 pb-2">
                <i class="fas fa-book text-red-600 dark:text-red-400"></i> ${window.I18N_COMMUNITY?.modals?.detail?.content_title || 'Contenido'}
            </h3>
...
function buildModalActions(actionsEl, lesson) {
    ...
    editBtn.innerHTML = `<i class="fas fa-edit"></i> <span class="hidden sm:inline">${window.I18N_COMMUNITY?.lessons?.suggest_edit || 'Sugerir Edición'}</span>`;
    ...
    if (globalThis.ContributionService?.isAdmin()) {
        ...
        historyBtn.innerHTML = `<i class="fas fa-history"></i> <span class="hidden sm:inline">${window.I18N_COMMUNITY?.modals?.detail?.history_btn || 'Historial'}</span>`;
...
async function showLessonHistory(id) {
    try {
        ...
        if (history.length === 0) {
            contentEl.innerHTML = `<p class="text-stone-500 text-center py-8">${window.I18N_COMMUNITY?.modals?.history?.no_history || 'No hay historial de versiones.'}</p>`;
            return;
        }
        ...
        const historyItems = history.map(v => `
            <div class="border-b border-stone-200 dark:border-stone-700 py-3 flex justify-between items-center">
                ...
                <button onclick="revertLesson('${id}', ${v.version})" class="text-xs bg-amber-100 text-amber-700 px-2 py-1 rounded hover:bg-amber-200">
                    ${window.I18N_COMMUNITY?.modals?.history?.restore_btn || 'Restaurar'}
                </button>
            </div>
        `).join('');
        ...
        contentEl.innerHTML = `
            <div class="mb-4">
                <button onclick="viewLesson('${id}')" class="text-sm text-red-500 hover:underline mb-2"><i class="fas fa-arrow-left"></i> ${window.I18N_COMMUNITY?.modals?.history?.back_btn || 'Volver a la lección'}</button>
                <h3 class="text-lg font-bold text-stone-800 dark:text-stone-100 mb-4">${window.I18N_COMMUNITY?.modals?.history?.title || 'Historial de Versiones'}</h3>
